...
export default ({ textOptions, holdWait = 3000, initialWait = 1500 }: DeletingTextProps) => {
    let currentTextOptionIndex = 0;
    const [visibleText, setVisibleText] = useState(textOptions[currentTextOptionIndex]);
    ...
    return <DeletingText className={cursorBlink ? 'cursor-blink' : ''}>{visibleText}</DeletingText>;
};
